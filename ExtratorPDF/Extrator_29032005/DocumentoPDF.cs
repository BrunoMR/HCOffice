...
namespace Extrator_29032005
{
    public class DocumentoPDF
    {
        ...
        public void Carregar(string pathPDF, int paginaInicial, string pathRepositorio)
        {
        ...
        public void ExtrairProcessos()
        {
            ...
            for (int pageNumber = this.PaginaPrimeiroProcesso; pageNumber <= this.PdfReader.NumberOfPages; pageNumber++)
            {
                ...
                // Verifica se existe processo ou apresentação validos na página e, caso não, passa para seguinte.
                // Motivo: Não percorrer página desnecessariamente.                    
                if (!Padrao_23072013.ContemProcesso(PageText) && !Padrao_23072013.ContemApresentacao(PageText))
                {
                    continue;
                }
                ...
                for (int i = 0; i <= split.GetUpperBound(0); i++)
                {
                    ...
                    #region Verifica conteúdo da linha

                    if (Padrao_23072013.ContemProcesso(_LINHA, out TextoRetorno))
                    {
                    ...
                    {
                        // Se objeto processo for nulo, fica em loop até encontrar um número de processo
                        // Motivo: Não executar validações desenecessariamente
                        if (processo == null)
                            continue;

                        if (Padrao_23072013.ContemApresentacao(_LINHA, out TextoRetorno))
                            processo.Apresentacao = TextoRetorno;
                    }

                    #endregion

                    // Valida se processo está completo
                    if (processo != null && processo.Apresentacao != null)
                    {
                        // Recupera página
        ...
        public void ExtrairProcessosNCL()
        {
            if (!this._carregou)
                throw new Exception("Nenhum arquivo PDf foi carregado.");
            ...
            for (int pageNumber = this.PaginaPrimeiroProcesso; pageNumber <= this.PdfReader.NumberOfPages; pageNumber++)
            {
                ...
                // Verifica se existe processo NCL válidos na página e, caso não, passa para seguinte.
                // Motivo: Não percorrer página desnecessariamente.                    
                if (!Padrao_23072013.ContemProcesso(PageText) && !Padrao_23072013.ContemNCL(PageText))
                {
                    continue;
                }
                ...
                for (int i = 0; i <= split.GetUpperBound(0); i++)
                {
                    ...
                    else
                    {
                        ...
                        if (Padrao_23072013.ContemNCL(_LINHA, out TextoRetorno))
                            processo.NCL = TextoRetorno;
                    }

                    #endregion

                    // Valida se processo está completo
                    if (processo != null && processo.NCL != null)
                    {
                        // Recupera página
                        Pagina pagina = this._paginas.Where(p => p.Numero == processo.Pagina).SingleOrDefault();

                        // Adiciona processo
                        pagina.ProcessosNCL.Add(processo);
                        processo = null;
                    }
                }
            }
            ...
            // Status
            this._extraiuProcessosNCL = true;
        }
                ...
                for (pageNumber = this.PaginaPrimeiroProcesso; pageNumber <= this.PdfReader.NumberOfPages; pageNumber++)
                {
                    ...
                    if (listener.Images.Count > 0)
                    {
                        foreach (var pair in listener.Images)
                        {
                            ...
                            string nomeArquivo =
                                string.Format("{0}_{1}.png", pageNumber.ToString("D8"), posicaoPagina.ToString("D2"));

                            // Gera imagem
                            pair.Key.Save(System.IO.Path.Combine(_pathImagens, nomeArquivo), ImageFormat.Png);

                            // Cria objeto de negócio da imagem
                            imagem = new Imagem { Pagina = pageNumber, Nome = nomeArquivo, Posicao = posicaoPagina };
        ...
        private int GetPaginaPrimeiroProcesso()
        {
            ...
            for (int pageNumber = 1; pageNumber <= this.PdfReader.NumberOfPages; pageNumber++)
            {
                ...
                if (Padrao_23072013.ContemApresentacao(PageText))
                {
                    pagina = pageNumber;
                    break;
                }
        ...
        private Cabecalho GetCabecalhoPagina()
        {
            ...
            for (int pageNumber = this._paginaPrimeiroProcesso; pageNumber <= this.PdfReader.NumberOfPages; pageNumber++)
            {
                ...
                // Verifica padrões
                if (Padrao_23072013.ContemData(_LINHA, out data) && Padrao_23072013.ContemNumeroRPI(_LINHA, out numeroRPI))
                    break;
            }
        ...
        #region Métodos arquivos antigos

        public void Carregar_OLD(string pathPDF, int paginaInicial, string pathRepositorio)
        {
            this._pdfReader = new PdfReader(pathPDF);
            ...
            if (paginaInicial == 0)
                this._paginaPrimeiroProcesso = this.GetPaginaPrimeiroProcesso_OLD();
            else
                this._paginaPrimeiroProcesso = paginaInicial;

            this._cabecalho = GetCabecalhoPagina_OLD();
        ...
        public void ExtrairProcessos_OLD(string procsIgnorados)
        {
            #region Validação
            ...
            List<string> listProcsIgnorados = new List<string>();

            // Processos ignorados
            if (!string.IsNullOrEmpty(procsIgnorados))
            {
                listProcsIgnorados =
                    procsIgnorados.Split(new Char[] { '|' }, StringSplitOptions.RemoveEmptyEntries).ToList();
            }
            ...
            for (int pageNumber = this.PaginaPrimeiroProcesso; pageNumber <= this.PdfReader.NumberOfPages; pageNumber++)
            {
                ...
                // Verifica se existe processo ou apresentação validos na página e, caso não, passa para seguinte.
                // Motivo: Não percorrer página desnecessariamente.                    
                //if (!Padrao_29032005.ContemProcesso(PageText) && !Padrao_29032005.ContemApresentacao(PageText))
                //{
                //    continue;
                //}
                ...
                for (int i = 0; i <= split.GetUpperBound(0); i++)
                {
                    ...
                    if (Padrao_29032005.ContemProcesso(_LINHA, out TextoRetorno))
                    {
                        if (listProcsIgnorados.Contains(TextoRetorno))
                        {
                            processo = null;
                            continue;
                        }
                    ...
                    else
                    {
                        ...
                        if (Padrao_29032005.ContemApresentacao(_LINHA, out TextoRetorno))
                            processo.Apresentacao = TextoRetorno;
                    }
        ...
        public void ExtrairImagens_OLD(string imgIgnoradas)
        {
            #region Validação
            ...
            #region Processos ignorados

            List<string> listImgIgnoradas = new List<string>();

            // Processos ignorados
            if (!string.IsNullOrEmpty(imgIgnoradas))
            {
                string[] split =
                    imgIgnoradas.Split(new Char[] { '|' }, StringSplitOptions.RemoveEmptyEntries);

                foreach (string nomeImagem in split)
                {
                    listImgIgnoradas.Add(nomeImagem + ".png");
                }
            }
            ...
            if (Directory.Exists(_pathImagens))
            {
                ...
                // Fica em loop até diretório ter sido excluído
                while (Directory.Exists(_pathImagens))
                { }
            }
                ...
                for (int pageNumber = this.PaginaPrimeiroProcesso; pageNumber <= this.PdfReader.NumberOfPages; pageNumber++)
                {
                    ...
                    if (listener.Images.Count > 0)
                    {
                        foreach (var pair in listener.Images)
                        {
                            ...
                            if (listImgIgnoradas.Contains(nomeArquivo))
                                continue;
                            ...
                            // Motivo: Imagens no formato Jpeg ficam invertidas
                            if (pair.Value == ".jpg")
                            {
                                //pair.Key.RotateFlip(RotateFlipType.Rotate180FlipX);
                                imagem.isJpeg = true;
                            }
                            ...
                            // Adiciona obj negócio imagem na página
                            pagina.Imagens.Add(imagem);

                            // Destrói obj negócio imagem
                            imagem = null;
                        }
        ...
        private Cabecalho GetCabecalhoPagina_OLD()
        {
            string PageText;
            string data = string.Empty;
            string numeroRPI = string.Empty;
            ...
            Cabecalho cabecalho =
                new Cabecalho { Data = DateTime.Now, NumeroRPI = this._filename };
            ...
            for (int pageNumber = this._paginaPrimeiroProcesso; pageNumber <= this.PdfReader.NumberOfPages; pageNumber++)
            {
                ...
                for (int i = 0; i <= split.GetUpperBound(0); i++)
                {
                    ...
                    // Verifica padrões
                    if (Padrao_29032005.ContemData(_LINHA, out data) && Padrao_29032005.ContemNumeroRPI(_LINHA, out numeroRPI))
                    {
                        // Se encontrou retorna e abandona rotina
                        cabecalho.Data = DateTime.Parse(data);
                        cabecalho.NumeroRPI = numeroRPI;
                        return cabecalho;
                    }
        ...
        private int GetPaginaPrimeiroProcesso_OLD()
        {
            string PageText;
            int pagina = 1;
            ...
            for (int pageNumber = 1; pageNumber <= this.PdfReader.NumberOfPages; pageNumber++)
            {
                ...
                if (Padrao_29032005.ContemApresentacao(PageText))
                {
                    pagina = pageNumber;
                    break;
                }
