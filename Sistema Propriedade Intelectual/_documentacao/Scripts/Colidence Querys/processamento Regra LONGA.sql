DECLARE
  ...
  @fileOfClient VARCHAR(2000)
	...
	SET @rpi = 2545
	SET @fileOfClient = 'C:\MarcasRPI\ListagemClienteTesteRPI2545.xlsx' --'D:\Guerra.xlsx'
  ...
  -- Query Executa processamento da colidência RPI Azure_New - Regra LONGA
	...
	SELECT
        PRC.ID_PROCESSO,
        PRO.NUMERO,
        dbo.RemoveCommonWords(PRO.MARCA, ISNULL(PRO.CLASSE_1,PRO.CLASSE_INTERNACIONAL)),
	...
	--select * from #CLIENT_PROCESSES_CLASS
	--select * from CLIENT_PROCESSES
	
	SELECT
      CLA.class,
      CLP.Processo,
	  dbo.RemoveCommonWords(CLP.Marca, CLA.class) Marca,
	  dbo.ORTOGRAFAR(dbo.RemoveCommonWords(CLP.Marca, CLA.class)) MarcaOrtografada
    INTO
    ...
    FROM
		...
		LEFT JOIN #CLIENT_PROCESSES_CLASS CPC ON CPC.Processo = CLI.Processo
        cross apply dbo.RadicalsProcessByWord(CLI.Processo, CPC.MarcaOrtografada, 0, 0) RAD
    --SELECT
    --    RAD.NUMERO_PROCESSO,
    --    RAD.RADICAL,
    --    RAD.LENGTH_RADICAL,
    ...
    FROM
        CLIENT_PROCESSES    CLI
		LEFT JOIN #CLIENT_PROCESSES_CLASS CPC ON CPC.Processo = CLI.Processo
		cross apply dbo.BuildBrandRadicalsByWord(CPC.Processo, CPC.Marca, 0, 1, CPC.class) RAD
    ...
    (Main, LengthRadical, Radical, NumeroProcesso, Marca, Classe, Deposito, Concessao, Processo, Titular, Pasta, Responsavel, Advogado, Class, MarcaSemVogais, LenMarcaSemVogais)
     SELECT
        DISTINCT
        PRRS.MAIN,
        PRRS.LENGTH_RADICAL,
